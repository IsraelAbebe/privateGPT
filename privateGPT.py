...
def main(embeddings_model_name, persist_directory, model_type, model_path, model_n_ctx, model_n_batch, target_source_chunks,mute_stream,hide_source):
    ...
    qa = RetrievalQA.from_chain_type(llm=llm, chain_type="stuff", retriever=retriever, return_source_documents= hide_source)
...
if __name__ == "__main__":
    ...
    mute_stream=conf.get('mute_stream', False)
    hide_source=conf.get('hide_source', True)

    

    # python privateGPT.py PERSIST_DIRECTORY=files/db1 MODEL_TYPE=OpenAI  MODEL_PATH=files/models/ggml-gpt4all-j-v1.3-groovy.bin  mute_stream=False hide_source=True
    print(embeddings_model_name, persist_directory, model_type, model_path, model_n_ctx, model_n_batch, target_source_chunks)
    main(embeddings_model_name, persist_directory, model_type, model_path, model_n_ctx, model_n_batch, target_source_chunks,mute_stream,hide_source)
